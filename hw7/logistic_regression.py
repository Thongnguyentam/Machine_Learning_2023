...
class CustomeLogisticRegression:
    ...
    def gradient(self, x, y_true, y_pred):
        ...
        weight = np.array([grad.mean() for grad in weight]) # the mean of each row of gradients_w to obtain the gradient of the loss with respect to each weight.
        return weight, bias
    ...
    def fit(self, x, y, epoch):
        ...
        for i in range(epoch):
            x_dot_weight = np.matmul(self.weights, x.T) + self.bias
            ...
            pred_to_class = [1 if p > 0.5 else -1 for p in pred]
    ...
    def predict(self, x):
        ...
        return [1 if p > 0.5 else -1 for p in probability]
